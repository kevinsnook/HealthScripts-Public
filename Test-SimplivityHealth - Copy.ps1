...
foreach ($OVCHost in $HostList){
    ...
    foreach ($Disk in $Disks){
        #"#Logical Disk $($Disk.Name) - $($Disk.serial_number) is $($Disk.Health)"
    ...
    #catch{}
    #write-host "Get Hard Disk done"
    #if ($Log) {Write-Logfile "$($SimplivityHost.HostName) -  Disk scan done"}
    #foreach ($Disk in $Disks){
